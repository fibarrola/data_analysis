...
print(data)
for question in ['quality', 'originality']:   
    for maker in ['User', 'CICADA']:
        fig = go.Figure()
        plot_data = [[] for name in names]
        for n, name in enumerate(names):
            for dat in data:
                if dat['question']==question and dat['name']==name and dat['maker']==maker:
                    if np.isnan(dat['score']):
                        plot_data[n].append(3)
                    else:
                        plot_data[n].append(int(dat['score']))
            print(plot_data[n])
            counts = [0 for k in range(5)]
            for m in plot_data[n]:
                counts[m-1] += 1
            counts = [c/21 for c in counts]
            fig.add_trace(
                go.Bar(
                    x=counts,
                    y=[f"{name} " for i in range(5)],
                    orientation='h',
                    marker={'color': colors1},
                )
            )
        fig.update_xaxes(range=[0,1],showgrid=False, visible=True)
        fig.update_layout(
            barmode='stack',
            showlegend=False,
            xaxis = {'tickformat': ',.0%'}
        ...
        # fig.show()
        plot4paper(fig, f'out/cicada_{question}{maker}.png', fontsize=24, height=300)
