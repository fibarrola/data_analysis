...
## BY QUESTION
fig = make_subplots(rows=18//N_COLS+1, cols=N_COLS, subplot_titles=questions, vertical_spacing=0.03)
    ...
    for o, option in enumerate(options):
        for q, question in enumerate(questions):
            ...
            for d in data:
                counts[d] += 1
            counts[0] = M-counts[1]-counts[2]-counts[3]-counts[4]/2
            fig.add_trace(go.Bar(x=counts, y=[f'Q{q+1}' for i in range(8)], orientation='h', marker={'color':colors}), row=s+1, col=o+1)
...
for q, question in enumerate(questions):
    for s, splitter in enumerate(splitters):
        ...
        fig.add_trace(go.Bar(x=[0], y=[' ' if splitter==splitters[1] else ''], orientation='h', marker={'color':colors}), row=q//N_COLS+1, col=q%N_COLS+1)

fig.add_vline(x=M)
fig.update_layout( barmode='stack', showlegend= False, height=H,  margin=dict(l=20, r=20, t=20, b=20))
fig.update_xaxes(range=[0,2*M], showgrid=False, visible=False)
fig.show()
